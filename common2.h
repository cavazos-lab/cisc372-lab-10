// Time-stamp: </w/c/overprop/common2.h, Sat, 14 Feb 2015, 12:10:24 EST, http://wrfranklin.org/>

// Common stuff for all my C++ programs.

// I keep adding things to this file.  Newer versions are mostly compatible.  If not, I
...
#else
const std::string boldblacktty("");
const std::string redtty("");
const std::string greentty("");
const std::string bluetty("");
const std::string magentatty("");
const std::string yellowbgtty("");
const std::string underlinetty("");
const std::string deftty("");
#endif


#define PRINT(arg)  #arg "= " <<(arg)     // Print an expression's name then its value,
					  // possibly followed by a comma or std::endl.
// Ex: std::cout << PRINTC(x) << PRINTN(y);
#define PRINTC(arg)  #arg << "= " << bluetty << (arg) << deftty << ", "
#define PRINTN(arg)  #arg << "= " << bluetty << (arg) << deftty << std::endl

// Assuming the expression is a qualified name, e.g., a.b, print the name after the dot etc.
...
void Print_Current_Process_Memory_Used() {
  ...
  pid_t p = getpid();
  sout << "cat /proc/" << p << "/status | grep Vm";
  cout << bluetty << sout.str() << deftty << endl;
  sout << std::ends;   // ends would make the logfile look binary.
